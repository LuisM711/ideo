...
//   "www.plandemejora.com" == url.host ||
//   "localhost" == url.host ||
//   window.location.replace("https://www.plandemejora.com");
...
class Interfaz {
  insertarFormularios(variable, restriccion) {
    ...
    (contenido += `<p id="no-negatividad">${noNegatividad.slice(
      0,
      -2
    )} ≥ 0</p><button type="submit" class="boton-dos-fases btn btn-success" id="resolver-Simplex">Resolver</button><button type="reset" class="boton-dos-fases btn btn-dark" id="reset-boton-problema">Limpiar</button>`),
    ...
    (contenido += `<h4>Función Objetivo</h4><p><span class="texto-simplex">${maxmin}:</span> Z = `),
      objetivo.forEach(function (element, index) {
        0 == index
          ...
          : index < nrovar
            ? (-1 == element.s
              ? (contenido += `- ${element.mul(-1).toFraction()}X<sub>${index + 1
                }</sub> `)
              : (contenido += `+ ${element.toFraction()}X<sub>${index + 1
                }</sub> `),
              (noNegatividad += `X<sub>${index + 1}</sub>, `))
            : index < nrovar + nrohol
              ? (-1 == element.s
    ...
    for (let i = 0; i < matriz.length; i++)
      i < matriz.length - 1
        ? (contenido += `<tr><td>${vcostes[
          psolucion[i][1]
        ].toFraction()}</td><th class="encabezado-tabla">${psolucion[i][0]
          }</th>`)
  ...
  explicacion(dosfases, signos, resultado) {
    ...
    let contenido = "";
    contenido +=
      "<p>El problema se adecuará al modelo estándar de programación lineal, agregando las variables de holgura, exceso y/o artificiales en cada una de las restricciones:</p><ul>";
    let h = 1,
      a = 1;
    for (let i = 0; i < signos.length; i++)
      "≤" == signos[i]
        ? resultado[i] >= 0
          ? ((contenido += `<li><strong>Restricción ${i + 1
            }:</strong> Tiene signo "≤" (menor igual) por lo que se agregará la variable de holgura S<sub>${h}</sub>.</li>`),
            h++)
          : ((contenido += `<li><strong>Restricción ${i + 1
            }:</strong> Tiene término independiente negativo. Se multiplicará por -1 a todos los coeficientes y se cambiará su signo de "≤" (menor igual) a "≥" (mayor igual). De esta forma le corresponde restar la variable de exceso S<sub>${h}</sub> y sumar la variable artificial A<sub>${a}</sub>.</li>`),
            h++,
            a++)
        : "≥" == signos[i]
          ? resultado[i] > 0
            ? ((contenido += `<li><strong>Restricción ${i + 1
              }:</strong> Tiene signo "≥" (mayor igual) por lo que se restará la variable de exceso S<sub>${h}</sub> y se sumará la variable artificial A<sub>${a}</sub>.</li>`),
              h++,
              a++)
            : ((contenido += `<li><strong>Restricción ${i + 1
              }:</strong> Tiene término independiente negativo o nulo. Se multiplicará por -1 a todos los coeficientes y se cambiará su signo de "≥" (mayor igual) a "≤" (menor igual). De esta forma le corresponde agregar la variable de holgura S<sub>${h}</sub>.</li>`),
              h++)
          : resultado[i] >= 0
            ? ((contenido += `<li><strong>Restricción ${i + 1
              }:</strong> Tiene signo "=" (igual) por lo que se agregará la variable artificial A<sub>${a}</sub>.</li>`),
              a++)
            : ((contenido += `<li><strong>Restricción ${i + 1
              }:</strong> Tiene término independiente negativo. Se multiplicará por -1 a todos los coeficientes y el signo "=" (igual) se mantiene. Se agregará la variable artificial A<sub>${a}</sub>.</li>`),
              a++);
    dosfases &&
      (contenido +=
        "</ul><p>El problema tiene variables artificiales por lo que utilizaremos el <strong>método de las 2 fases</strong>. En la primera fase, la función objetivo busca <strong>minimizar la suma de las variables artificiales</strong></p> "),
      (contenido +=
  ...
  enlaces(objeto) {
    ...
    let texto = `\n    <button id="copiar-enlace" style="background-color:#60A5FA;display:inline-flex;align-items: center;min-width:300px;justify-content: center;" class="boton-dos-fases" type="button">\n    <svg id="icono-enlace" xmlns="http://www.w3.org/2000/svg" style="width:1.15rem;heigth:1.15rem;margin-right:10px" viewBox="0 0 20 20" fill="currentColor">\n      <path fill-rule="evenodd" d="M12.586 4.586a2 2 0 112.828 2.828l-3 3a2 2 0 01-2.828 0 1 1 0 00-1.414 1.414 4 4 0 005.656 0l3-3a4 4 0 00-5.656-5.656l-1.5 1.5a1 1 0 101.414 1.414l1.5-1.5zm-5 5a2 2 0 012.828 0 1 1 0 101.414-1.414 4 4 0 00-5.656 0l-3 3a4 4 0 105.656 5.656l1.5-1.5a1 1 0 10-1.414-1.414l-1.5 1.5a2 2 0 11-2.828-2.828l3-3z" clip-rule="evenodd" />\n    </svg>\n    <svg id="icono-check" xmlns="http://www.w3.org/2000/svg" style="width:1.15rem;heigth:1.15rem;margin-right:10px;display:none;" fill="none" viewBox="0 0 24 24" stroke="currentColor">\n      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" />\n    </svg>\n      Copiar enlace de resultados\n    </button>\n    <ul style="text-align: left;margin-top:35px;">\n    <li>Resolver el problema con el <a href="https://www.plandemejora.com/calculadora-metodo-m-grande/?problem=${enlace}" target="_blank" rel="noopener noreferrer" >método de la Gran M</a>.</li>`;
    texto = '';
    if (2 === objeto.co.length) {
      const objeto2 = JSON.parse(JSON.stringify(objeto));
      (objeto2.rr = objeto2.r),
        (objeto2.r = objeto2.cr),
        delete objeto2.cr,
        (objeto2.o = objeto2.o.slice(0, 3).toLowerCase());
      const enlace2 = reducir(objeto2);
      //   texto +=
      //     '<li>Resolver el problema con el <a href="https://www.plandemejora.com/calculadora-metodo-grafico-programacion-lineal/?problem=' +
      //     enlace2 +
      //     '" target="_blank" rel="noopener noreferrer" >método gráfico</a>.</li>';
    ...
    (texto += "</ul>"),
      ...
      divResultados.appendChild(div)
    // document
    //   .getElementById("copiar-enlace")
    //   .addEventListener("click", (e) => {
    //     const iconoEnlace = document.getElementById("icono-enlace"),
    //       iconoCheck = document.getElementById("icono-check");
    //     navigator.clipboard.writeText(host + "?problem=" + enlace),
    //       (e.target.style.backgroundColor = "#266cb0"),
    //       (iconoEnlace.style.display = "none"),
    //       (iconoCheck.style.display = "block"),
    //       setTimeout(() => {
    //         (e.target.style.backgroundColor = "#60A5FA"),
    //           (iconoEnlace.style.display = "block"),
...
class Problema {
  constructor(
    ...
    if (this.dosfases) {
      ...
      for (let i = 0; i < this.holguras.length; i++)
        this.posicion_costes[this.posicion_costes.length] = `S<sub>${i + 1
          }</sub>`;
      for (let i = 0; i < this.artificiales.length; i++)
        this.posicion_costes[this.posicion_costes.length] = `A<sub>${i + 1
          }</sub>`;
...
function crearFormularioCoeficientes() {
  ...
  // btn.type = "submit";
  // btn.id = "resolver-Simplex";
  // btn.classList.add("boton-dos-fases", "btn", "btn-primary");
  // btn.innerText = "Resolver";
  // formCoeficientesProblema.after(btn);
}
    ...
    if (problema.multiple()) {
      ...
      coeficientesObjetivo.forEach(function (element, index) {
        0 == index
          ...
          : -1 == element.s
            ? (recta += `- ${element.mul(-1).toFraction()}X<sub>${index + 1
              }</sub> `)
            : (recta += `+ ${element.toFraction()}X<sub>${index + 1}</sub> `);
      }),
